...
fun VentanaBuscar(
    ...
    Column(
        ...
        Button(
            onClick = {
                msgError = ""
                val textIdIntro = textId.trim()
                val textNombreProductoIntro = textNombreProducto.trim()

                when {
                    textIdIntro.isNotEmpty() -> {
                        val id = textIdIntro.toIntOrNull()
                        if (id != null) {
                            scope.launch {
                                try {
                                    ...
                                    Log.e(MainActivity.personalLogs+" - VentanaBuscar", msgError, e)
                    ...
                    textNombreProductoIntro.isNotEmpty() -> {
                        scope.launch {
                            inventoryViewModel.getItemsByNameLike(textNombreProductoIntro)
                                .collect { result ->
                                    items = result
                                }
                        }
